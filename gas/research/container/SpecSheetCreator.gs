...
function createUserSpecSheet(ss) {
  ...
  const faqs = [
    {
      ...
      a: 'ヘッダー行（1行目）に「SKU」列が存在するか確認してください。'
    },
    {
  ...
  const troubles = [
    ...
    },
    {
      ...
      checks: [
        '原因: 出品シートの列構造が変わった',
        '解決: このツールはヘッダー名でマッピングするため、列を追加・削除しても自動で対応します。ヘッダー行（1行目）のヘッダー名を変更しないでください。'
      ]
    }
  ];
  ...
  const techFeatures = [
    {
      ...
      description: '出品シートのヘッダー行（1行目）から列マッピングを動的に構築',
      benefits: [
