...
const ITEM_CACHE_HEADERS    = ['item_url', 'category_id', 'category_name', 'item_specs_json', 'fetched_at'];
...
function getOrCreateCacheSheet() {
  ...
  if (!sheet) {
    ...
    sheet.setColumnWidth(5, 160); // fetched_at
...
/**
 * _cache シートから item_url でエントリを検索
 *
 * @param {string} url
 * @returns {{categoryId: string, categoryName: string, specifics: Object}|null}
 *          ヒットしない場合は null
 */
function getCacheEntry(url) {
  try {
    ...
    const data = sheet.getRange(2, 1, lastRow - 1, ITEM_CACHE_HEADERS.length).getValues();
    const target = normalizeCacheUrl(url);

    for (let i = 0; i < data.length; i++) {
      if (normalizeCacheUrl(String(data[i][CACHE_COL.ITEM_URL])) === target) {
        const specsJson = String(data[i][CACHE_COL.ITEM_SPECS_JSON] || '{}');
        let specifics = {};
        try {
          specifics = JSON.parse(specsJson);
        } catch (e) {
          Logger.log('[Cache] item_specs_json パースエラー（行' + (i + 2) + '）: ' + e.toString());
        }
        Logger.log('[Cache] ヒット: ' + url);
        return {
          categoryId:   String(data[i][CACHE_COL.CATEGORY_ID]   || ''),
          categoryName: String(data[i][CACHE_COL.CATEGORY_NAME]  || ''),
          specifics:    specifics
        };
      }
    }
...
/**
 * _cache シートにエントリを保存（同一URLは上書き、新規は追記）
 *
 * @param {string} url
...
function saveCacheEntry(url, productInfo) {
  try {
    ...
    const lastRow = sheet.getLastRow();
    if (lastRow >= 2) {
      const urlCol = sheet.getRange(2, 1, lastRow - 1, 1).getValues();
      for (let i = 0; i < urlCol.length; i++) {
        if (normalizeCacheUrl(String(urlCol[i][0])) === target) {
          sheet.getRange(i + 2, 1, 1, newRow.length).setValues([newRow]);
          Logger.log('[Cache] 更新: ' + url);
          return;
        }
      }
    }
